...
(defun k/normalize-keywords (kw &optional sw)
  "Rules for getting keyword symbols.
Returns the keys for the symbols in the alist, in the correct order."
  (let ((sw (chops sw)))
    (-as-> (list kw sw) kws
           ...
           (if (string= (car kws) "end")
             ...
             kws)
           (-replace "begin" "block" kws)
           (-replace "end" "block" kws)
